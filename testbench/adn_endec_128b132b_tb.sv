...
module adn_endec_128b132b_tb;
  ...
  adn_endec_encoder_128b132b encoder_dut (
  ...
  adn_endec_decoder_128b132b decoder_dut (
  ...
  task automatic check_stream_block;
    ...
    begin
      ...
      if (enc_header_err != ((sync_header == 4'b0000) || (sync_header == 4'b1111))) begin
        $display("FAIL 128b132b encoder header_err mismatch header=%b", sync_header);
        note_case(0);
      end else begin
        note_case(1);
      end
      if (dec_header_err != ((sync_header == 4'b0000) || (sync_header == 4'b1111))) begin
        $display("FAIL 128b132b decoder header_err mismatch header=%b", sync_header);
        note_case(0);
      end else begin
        note_case(1);
      end
      if (dec_payload_out != payload) begin
        $display("FAIL 128b132b payload mismatch");
      ...
      if (dec_sync_header_out != sync_header) begin
        $display("FAIL 128b132b header mismatch exp=%b got=%b", sync_header, dec_sync_header_out);
        note_case(0);
      end else begin
        note_case(1);
      end
      if (dec_state_out != enc_state_out) begin
        $display("FAIL 128b132b state mismatch exp=%06h got=%06h", enc_state_out, dec_state_out);
        note_case(0);
      end else begin
        note_case(1);
      end
  ...
  initial begin
    ...
    check_stream_block(128'h0123456789ABCDEFFEDCBA9876543210, 4'b0001);
    saved_valid_block   = enc_block_out;
    saved_valid_state   = enc_state_in;
    saved_valid_payload = enc_payload_in;
    check_stream_block(128'h000102030405060708090A0B0C0D0E0F, 4'b0010);
    check_stream_block(128'h00112233445566778899AABBCCDDEEFF, 4'b0101);
    check_stream_block(128'hFFEEDDCCBBAA99887766554433221100, 4'b0000);
    check_stream_block(128'hDEADBEEFCAFEBABE1122334455667788, 4'b1111);

    dec_block_in = {4'b1111, saved_valid_block[127:0]};
    dec_state_in = saved_valid_state;
    #1;

    if (!dec_header_err) begin
      $display("FAIL 128b132b corrupted header should raise header_err");
      note_case(0);
    end else begin
      note_case(1);
    end
    if (dec_payload_out != saved_valid_payload) begin
      $display("FAIL 128b132b corrupted-header payload mismatch");
    ...
    $display("128b132b encoder/decoder self-check passed");
    $finish;
  end
